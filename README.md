...
```bash
npx pingme-cli init       # Setup pingme
npx pingme-cli test       # Send a test SMS
npx pingme-cli uninstall  # Remove pingme
npx pingme-cli --version  # Show version
npx pingme-cli --help     # Show help
```
...
1. **Installation**: When you run `npx pingme-cli init`, pingme creates:
...
To update or remove credentials, run `npx pingme-cli init` again or `npx pingme-cli uninstall`.

## Troubleshooting

### SMS not sending

1. **Verify credentials**: Run `npx pingme-cli test` to send a test message
